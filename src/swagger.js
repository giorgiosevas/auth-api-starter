...
A production-ready authentication API built with Node.js, Express, and PostgreSQL.
...
- **Protected Routes** - Access user profile with JWT

### Authentication Flow
1. Register or Login to get \`accessToken\` and \`refreshToken\`
2. Use \`accessToken\` in Authorization header: \`Bearer <token>\`
3. When access token expires, use \`/refresh\` endpoint with \`refreshToken\`
4. On logout, invalidate the refresh token
          ...
          properties: {
            ...
            data: {
              ...
              properties: {
                ...
                accessToken: {
                  type: 'string',
                  description: 'JWT access token (expires in 15min)',
                },
                refreshToken: {
                  type: 'string',
                  format: 'uuid',
